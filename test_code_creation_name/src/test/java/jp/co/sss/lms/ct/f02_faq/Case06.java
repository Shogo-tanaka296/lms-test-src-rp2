...
public class Case06 {
	...
	void test05() {
		int count = webDriver.findElements(By.cssSelector("fieldset a")).size();
		for(int i=0 ; i<count ; i++) {
			//カテゴリーの<a>要素をすべて取得してリストに格納
			List<WebElement> categoryList =webDriver.findElements(By.cssSelector("fieldset a"));
			//エビデンスに使用する文字列を取得(画面遷移すると取得できなくなるため)
			String questionName = categoryList.get(i).getText();
			//要素をクリック
			categoryList.get(i).click();
			//検索結果の要素を全件取得（取得できなければ空のリストを返す)
			List<WebElement> questions = 
					webDriver.findElements(By.cssSelector("dl[id^='question-h']"));
			//検索結果のリストが空ではないか
			if(questions.size()>0) {
				//リストをループして検証
				for(WebElement value : questions) {
					assertTrue(value.isDisplayed());
				}
			}//if終わり
			//スクリーンショットによるエビデンス取得
			getEvidence(new Object() {},questionName + "検索結果");
			//スクロールした分元に戻す
			scrollTo("-600");
	...
	void test06() {
		...
		for(int i=0 ; i<count ; i++) {
			...
			if(questions.size()>0) {
				...
				for(WebElement question : questions) {
					...
					List<WebElement> answers =
							webDriver.findElements(By.cssSelector("dd[id='answer-h']"));
					//展開した要素でリストをループ（回答）
			...
			//スクリーンショットによるエビデンス取得
			getEvidence(new Object() {},questionName + "回答");
			//スクロールした分元に戻す
			scrollTo("-600");
			//FAQページに戻る
			webDriver.navigate().back();
